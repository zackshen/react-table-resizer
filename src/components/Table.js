...
export default function MyTable({ loading, infinite, ...props }) {
  ...
  const getFinalWidth = col => {
    let width = col.width || 0;

    if (col.minWidth && width < col.minWidth) width = col.minWidth;

    if (col.maxWidth && width > col.maxWidth) width = col.maxWidth;
    ...
    pagination = pageOptions.length ? (
      <Pagination {...getRowProps()}>
        <Cell>
          ...
          >
            {[10, 20, 30, 40, 50, 100, 200, 300, 500, 1000].map(pageSize => (
