...
export const useColumns = props => {
  ...
  const { columns, headerGroups, headers } = useMemo(() => {
    ...
    if (resizedColumns) {
      ...
      Object.entries(resizedColumns).forEach(([key, value]) => {
        const found = updatedColumns.find(col => col.id === key);
  ...
  }, [groupBy, userColumns, resizedColumns]);

  return {
    ...props,
    originalColumns: userColumns.map(col => ({ ...col })),
    columns,
    headerGroups,
    headers
  };
