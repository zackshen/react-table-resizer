...
export const useResizer = props => {
  ...
  const resizeColumn = e => {
    ...
    resizingInfo.current.currentWidth =
      bodyCells && bodyCells[0] && bodyCells[0].getBoundingClientRect().width;
    ...
    //  */
    if (currentWidth - updatedWidth >= -0 && currentWidth - updatedWidth <= 0)
      return;
    ...
    if (groupedCells)
      groupedCells.forEach(cell => {
        ...
        const updateHeaderdWidth = childRefs.reduce((prev, current) => {
          prev += (current && current.getBoundingClientRect().width) || 0;
